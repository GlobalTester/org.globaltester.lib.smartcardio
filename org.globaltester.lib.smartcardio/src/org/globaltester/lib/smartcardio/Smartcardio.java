package org.globaltester.lib.smartcardio;
...
import org.globaltester.lib.smartcardio.Winscard.Dword;
import org.globaltester.lib.smartcardio.Winscard.DwordByReference;
import org.globaltester.lib.smartcardio.Winscard.SCardReaderState;
...
public class Smartcardio extends Provider {
	...
	public static class JnaCardChannel extends CardChannel {
		...
		private ByteBuffer transmitImpl(byte[] command, ByteBuffer response) throws CardException, JnaPCSCException {
			...
			//changed implementation for GT: if status word is 61xx do net send getResponse command automatically. if status word is 6Cxx do not send new request with correct length.
			/*
			//Don't loop forever.
			for (int i=0; i<8; i++) {
